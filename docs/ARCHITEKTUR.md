...
| Target level (B2/C1) changes only the planner's band cap | A second C1 exam blueprint; raising `TargetDifficulty` too | The app has Goethe-B2 content and a B2 blueprint; inventing a C1 exam would promise something it cannot assess. So C1 lifts `RankFocusNodes`' cap to `CefrBand.C1` - the three C1 nodes (elevated vocabulary, elevated sentence linking, fixed phrases) and the B2.2 grammar reach the focus slot - while placement, readiness and the exam pages stay B2 and say so. Difficulty targeting stays tied to measured ability: aiming higher must not mean being handed items far above one's level. Heute gains a "C1-Nähe" line (mastery of the C1/B2.2 nodes), deliberately not called readiness. |
| Occupation as a tie-break plus a small node boost | Separate content tracks per field; filtering the bank to one field | The learner's gaps decide what is practised; the field only decides *which* of several equally suitable items shows up, and nudges a couple of vocabulary nodes by 0.15. Cheap, needs no new content, and cannot hide material or let someone drill only their comfort zone. `Unspecified` is a no-op by construction (the tie-break key is constant), so the pre-existing planner tests keep their meaning. Reasons name it when it decided, because an invisible adaptation reads as randomness. |
| Name tokens in the content, substituted at the render boundary | Storing a per-learner copy of the content; string replacement inside the catalogue | The catalogue is one singleton shared by every account, so it must stay neutral - a name written into it would leak between learners. `NameTemplate.Render` produces copies where content is handed to one person (`LearningService` for exercises, `Lektion` for the story). `Exercise.Answers` is excluded: the checker compares the learner's input against those strings verbatim, so a token there would make an exercise unsolvable; a test asserts none appears. Empty name fields fall back to a stand-in drawn from a small pool by a stable hash of the account id (`NameFallback`, FNV-1a rather than `GetHashCode`, which is randomised per process and would rename the same learner after every restart). The author's own name is deliberately *not* the default: this repository is public, and a stranger's course greeting them as its author is the wrong behaviour. The pool is male because nineteen places in the content say "Herr {Nachname}" - widening it means making those salutations variable first. |
| Time budget per answer belongs to the provider preset (`TutorPreset.DefaultTimeoutSeconds`), editable per account | One global constant; a timeout derived from the model name | The spread is not a preference but a measurement: a hosted 70B answers a full evaluation in seconds, a local 7B needs one to three minutes for the same ~800 output tokens (6.7 tok/s on a desktop GPU). One flat 120 s cut the local case off three seconds before the end, and the learner saw "Zeitüberschreitung" after two minutes of waiting. Local presets therefore start at 300 s, hosted at 120 s, and `TutorSettings.ForPreset` carries model *and* budget together so switching provider never leaves the previous one's number behind. `Draft()` on the settings page must pass every field of the record - it used to omit `Effort` and `TimeoutSeconds`, which silently reset both on each save. |
| Motivation from FSRS state and a minutes goal | XP, badges, levels, leaderboard | One learner per installation: there is nobody to rank against, and a points economy would compete with the honest numbers rather than support them. The three cards read state that already exists (stability, due dates, retrievability), so nothing extra is tracked. The one thing not shown is "newly stuck this week" - past stabilities are not stored, so the crossing point is unknowable and the card says "reviewed this week" instead. |
| Readiness snapshots written when the dashboard is opened | A background job; recomputing history from attempts | There is no scheduler in this app and no server to run one on; the dashboard is the moment the number exists anyway, and a reading nobody looked at need not be kept. Recomputing the past from attempts would require replaying the whole model, and its output would change whenever the model does - a "trend" that moves because the formula moved is worse than none. The comparison window is 6-8 days rather than exactly 7 for the same reason the streak forgives today: people skip days. |
