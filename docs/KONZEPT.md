...
The language is decomposed into **48 skill nodes** across seven areas (Grammatik, Wortschatz, Redemittel, Lesen, Hören, Schreiben, Sprechen), each placed on a CEFR sub-band (B1.1 … B2.2). Exercises point at nodes; **54 error codes** point at nodes. The learner is described by one ability estimate per node – nothing is ever "lesson 7 of 30".
