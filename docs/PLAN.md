...
- ✅ Listening with one voice per speaker, 0.9.0: `SpeakerTurns` splits a conversation, `SpeakerVoices.ForScript` assigns the voices (strangers alternate), names are not spoken; the generator may now produce Teil 3 (moderator + two guests). Still ⬜: authored three-speaker Teil 3 texts in the content bank (today one two-person Teil 3, `hv.mc.102`).
...
- ✅ Revision of 2026-09-08 (five review agents + a UI walk-through), 0.9.0: readiness only from the module's own evidence (no number, snapshot or trend without it); answer keys balanced at load time (`KeyShuffle`); dead hosted model IDs replaced; tutor error paths (Claude timeout/refusal/max-tokens, OpenAI-compatible `temperature`, `Retry-After`, `finish_reason`); Lesen/Hören before the drills with 35 % of the budget; dark-mode contrast, real `<h1>`s, 40-px read-aloud targets; `appsettings.Local.json` loaded, DPAPI-protected key ring; HttpClient lifetime, reset clears snapshots, orphaned session steps healed; speech recognition restarts after pauses, typed drafts survive recording, four analyser false positives; exam rules (`ExamTiming`: plays per Hören part, time per Lesen part, remaining time for Schreiben/Vortrag).
