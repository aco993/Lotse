...
public static class NameFallback
{
    ...
    // No surname of a story character (Krüger, Kaya): the learner must not share a name with someone they talk to.
    private static readonly string[] LastNames =
        ["Berger", "Hoffmann", "Wagner", "Keller", "Brandt", "Lehmann", "Vogel", "Sommer", "Winkler", "Reuter", "Bauer", "Neumann"];

    /// <summary>Last resort where no account is at hand at all (a preview, a test, a page rendered before login).</summary>
    public static readonly (string First, string Last) Neutral = ("Lukas", "Berger");
    ...
    public static (string First, string Last) For(string? accountId)
    {
        ...
        return (FirstNames[(int)(h % (uint)FirstNames.Length)],
                LastNames[(int)(h / 16 % (uint)LastNames.Length)]);
    }
